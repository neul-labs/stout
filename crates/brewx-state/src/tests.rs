...
fn test_installed_packages_get() {
    ...
    assert_eq!(pkg.installed_by, "brewx");
}
...
fn test_paths_new() {
    let tmp = tempdir().unwrap();
    let brewx_dir = tmp.path().join("brewx");
    let prefix = tmp.path().join("homebrew");

    let paths = Paths::new(brewx_dir.clone(), prefix.clone());

    assert_eq!(paths.brewx_dir, brewx_dir);
